            ...
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <h6 v-if="!!selected">Tem certeza que deseja excluir a marca "{{ selected.nome }}"?</h6>
                </div>
...
export default defineComponent({
  ...
  setup () {
    ...
    const { entities, fetchEntities, columns } = usePagination('/produtos/marcas', [{
      key: 'id',
      label: '#',
      type: 'numeric'
    }, {
    ...
    const onSubmit = handleSubmit(async ({ nome }) => {
      try {
        if (!selected.value?.id) {
          await execute('POST', '/produtos/marcas', {
            nome
          })
        } else {
          await execute('PATCH', '/produtos/marcas', {
    ...
    const excluir = async () => {
      try {
        await execute('DELETE', '/produtos/marcas', {
          id: selected.value.id
        })
